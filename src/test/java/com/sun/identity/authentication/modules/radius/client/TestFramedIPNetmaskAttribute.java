...
public class TestFramedIPNetmaskAttribute {
    ...
    public void testFromOctets() {
        byte[] o = new byte[6];
        o[0] = Attribute.FRAMED_IP_NETMASK;
        o[1] = 6;
        o[2] = 5;
        o[3] = 4;
        o[4] = 3;
    ...
    public void testFromAddress() throws IOException {
        ...
        Assert.assertEquals(a.getMask()[2], 3);
        Assert.assertEquals(a.getMask()[3], 2);
        Assert.assertEquals(a.getValue()[0], Attribute.FRAMED_IP_NETMASK);
        Assert.assertEquals(a.getValue()[1], 6);
        Assert.assertEquals(a.getValue()[2], 5);
        Assert.assertEquals(a.getValue()[3], 4);
        Assert.assertEquals(a.getValue()[4], 3);
        Assert.assertEquals(a.getValue()[5], 2);
    }
}
