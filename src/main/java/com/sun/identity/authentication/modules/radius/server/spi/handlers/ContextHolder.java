 ...
 *
 * Created by markboyd on 11/28/14.
 */
public class ContextHolder {
     ...
     * to -1 so that we can centralize updating this info set without regard to whether we are handling the first set
     * or following sets.
     */
     ...
     * accessChallenge response excluding the undeclared PagePropertiesCallback object that is always first in the
     * array of callbacks and contains the header for the html page in which the fields for this set of callbacks are
     * presented for html clients.
     */
    public int idxOfCurrentCallback = 0;

    /**
     * The zero based index of the set of callbacks within a given module. Modules can have more than one set of callback
     * with a single set translating to a single web page when openam is used for web authentication.
     */
     ...
     * times the timeout value for that set. At creation time we instantiate to one minute so that the holder
     * won't get purged from cache between getting created and loading of the first callback set.
     */
     ...
     * attempt was aborted by that user or they took too long to complete a given step. When System.currentTimeMillis
     * is greater than this value the expiration point has passed and the item should be purged. This value may
     * change multiple times during a given authentication process depending on how many pages of callbacks are
     * incurred. Each set of callbacks has its own declared number of seconds allows for response and that value
     * will be set here when that callback set is incurred.
     */
