...
public class OpenAMAuthHandler implements AccessRequestHandler {
    private static final Logger LOG = Logger.getLogger(OpenAMAuthHandler.class.getName());
     ...
     */
    private static final Map<String, ContextHolder> CONTEXT_CACHE = new HashMap<String, ContextHolder>();
        ...
        public void terminate() {
            ...
            if (t != null) {
                ...
                while (OpenAMAuthHandler.sweeper != null) {
                    LOG.log(Level.INFO, "Waiting for " + name + " to exit.");
                    try {
                        Thread.sleep(200);
                    } catch (InterruptedException e) {
                        LOG.warning("Thread sleep interupted.");
                    }
                }
            }
        }
    ...
    private static class CacheSweeper implements Runnable {
        ...
        @Override
        public void run() {
            LOG.log(Level.INFO, Thread.currentThread().getName() + " thread started.");
            ...
            while (notInterrupted) {
                ...
                synchronized (CONTEXT_CACHE) {
                    for (Iterator<Map.Entry<String, ContextHolder>> itr = CONTEXT_CACHE.entrySet().iterator(); itr
                            .hasNext();) {
                ...
                if (purgedItems > 0) {
                    LOG.log(Level.INFO,
                            "Purged " + purgedItems + " items from " + OpenAMAuthHandler.class.getSimpleName()
                                    + " cache.");
                }
            }
            LOG.log(Level.SEVERE, "Thread " + Thread.currentThread().getName() + " interrupted. Exiting.");
            sweeper = null;
        }
    }
     ...
     *
     * @return the number of items in teh context cache.
     */
    ...
    public void handle(AccessRequest request, RadiusResponseHandler respHandler) {
        ...
        if (state != null) {
            ...
            holder = CONTEXT_CACHE.get(cacheKey);
        }
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Unable to extract credential field from RADIUS request. Denying Access.", e);
     ...
     *
     * @return the shutdown listener instance.
     */
    @Override
    ...
    private void gatherUserInput(RadiusResponseHandler respHandler, ContextHolder holder, String answer,
        ...
        while (holder.authPhase == ContextHolder.AuthPhase.GATHERING_INPUT) {
            if (holder.callbacks == null) {
                ...
                boolean injected = injectAnswerForCallback(respHandler, holder, answer); // answers always come through
            ...
            if (holder.idxOfCurrentCallback > holder.callbacks.length - 1) {
                ...
                try {
                    ...
                    LOG.log(Level.SEVERE, "Exception thrown while submitting callbacks. Rejecting access.", t);
                    rejectAccessAndTerminateProcess(respHandler, holder);
                    return;
                }
     ...
     * caller. Sets holder.authPhase = TERMINATED if something happened causing the authentication process to fail.
     *
     * @param context
     * @param holder
     * @return
    ...
    private boolean isNextCallbackSetAvailable(RadiusResponseHandler context, ContextHolder holder) {
        ...
        if (holder.callbacks[0] instanceof PagePropertiesCallback) { // not a formal callback, openam specific
            ...
            LOG.log(Level.SEVERE, "Callback at index 0 is not of type PagePropertiesCallback!!!");
        ...
        if (httpCbIncurred || redirectCbIncurred) {
            LOG.log(Level.SEVERE, "Radius can not support "
                    + (httpCbIncurred ? HttpCallback.class.getSimpleName() : RedirectCallback.class.getSimpleName())
                    + " used by module " + holder.chainModuleIndex + " with name " + holder.moduleName + " in chain '"
    ...
    private void terminateAuthnProcess(ContextHolder holder) {
        synchronized (CONTEXT_CACHE) {
            CONTEXT_CACHE.remove(holder.cacheKey);
        }
    ...
    private boolean injectAnswerForCallback(RadiusResponseHandler respHandler, ContextHolder holder, String answer) {
        ...
        if (cb instanceof NameCallback) {
            ...
            LOG.log(Level.SEVERE, "Unrecognized callback type '" + cb.getClass().getSimpleName()
    ...
    private int parseInt(String intVal, String answer, int maxIdx, ContextHolder holder, Callback cb,
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Invalid number '" + intVal + "' specified in answer '" + answer + "' for callback "
                    + holder.idxOfCurrentCallback + " of type " + cb.getClass().getSimpleName() + " for callback set "
                    + holder.idxOfCallbackSetInModule + " in module " + holder.chainModuleIndex
                    + (holder.moduleName != null ? " with name " + holder.moduleName : "")
                    + " of authentication chain " + authChain + " in realm " + realm + ". Denying Access.");
            rejectAccessAndTerminateProcess(respHandler, holder);
            return idx;
        }
        if (idx < 0 || idx > maxIdx) {
            LOG.log(Level.SEVERE, "Out of range index specified in answer '" + answer + "' for callback "
                    + holder.idxOfCurrentCallback + " of type " + cb.getClass().getSimpleName() + " for callback set "
                    + holder.idxOfCallbackSetInModule + " in module " + holder.chainModuleIndex
                    + (holder.moduleName != null ? " with name " + holder.moduleName : "")
     ...
     * callbacks. Returns true if authentication was started and user requirements beyond usernamd and password can now
    ...
    private ContextHolder startAuthProcess(RadiusResponseHandler respHandler, Map<Class, Attribute> reqAttsMap,
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Unable to start create " + AuthContext.class.getName() + ". Denying Access.", e);
            rejectAccessAndTerminateProcess(respHandler, holder);
            return holder;
        }
        ...
        try {
            ...
            LOG.log(Level.SEVERE, "Unable to start login process. Denying Access.", e);
            rejectAccessAndTerminateProcess(respHandler, holder);
            return holder;
        }
        ...
        if (!isNextCallbackSetAvailable(respHandler, holder)) {
            ...
            if (holder.authPhase != ContextHolder.AuthPhase.TERMINATED) {
                LOG.log(Level.SEVERE, "Unable to start login process. No callbacks available. Denying Access.");
                rejectAccessAndTerminateProcess(respHandler, holder);
            }
            return holder;
        }
    ...
    /**
     * Creates a new ContextHolder and associated unique cache key, injects the holder into the cache, and returns the
     * holder.
     *
     * @return
     */
    private ContextHolder newContextHolder() {
        synchronized (CONTEXT_CACHE) {
            while (true) {
                String key = Integer.toHexString(SECURE_RANDOM.nextInt());
                if (!CONTEXT_CACHE.containsKey(key)) {
                    ContextHolder holder = new ContextHolder(key);
                    CONTEXT_CACHE.put(holder.cacheKey, holder);
                    return holder;
                }
            }
        }
    ...
    private ReplyMessageAttribute getNextCallbackReplyMsg(RadiusResponseHandler respHandler, ContextHolder holder) {
        ...
        // JPB - replaced following line as checkstyle and auto-formatting made this line too long.
        // String header = (holder.callbackSetProps != null && !"".equals(holder.callbackSetProps.getHeader()) ?
        // holder.callbackSetProps.getHeader() + " ": "");
        String header = "";
        if (holder.callbackSetProps != null && !"".equals(holder.callbackSetProps.getHeader())) {
            header = holder.callbackSetProps.getHeader() + " ";
        }
        ...
        if (cb instanceof NameCallback) {
            ...
            LOG.log(Level.SEVERE, "Radius can not support " + cb.getClass().getSimpleName() + " used by module "
                    + holder.chainModuleIndex + " with name " + holder.moduleName + " in chain '" + this.authChain
                    + "'. Denying Access.");
            rejectAccessAndTerminateProcess(respHandler, holder);
        }
        return msg;
