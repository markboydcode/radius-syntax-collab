 ...
 * them in configuring our transitions.
 *
...
public enum State {
    ...
    VERIFY_SMS_PHONE_NUMBER, // prompts user that an sms text was sent to their indicated numbrer and must be entered to verify (pair it with that user) and be authorized

    AWAIT_SMS_DELIVERED_PASSCODE, // prompts user that an sms text was sent to their previously registered numbrer and must
    // be entered for authorization

    GET_VOICE_PHONE_NUMBER,

    VERIFY_VOICE_PHONE_NUMBER, // propts user that voice delivered passcode has been sent and must be entered to verify phone number

    AWAIT_VOICE_DELIVERED_PASSCODE, // prompts user that an voice delivered passcode was sent to their previously registered numbrer and must
    // be entered for authorization
