...
The code in this repo contains implements three distinct features that share code for speaking the RADIUS protocol. First,
...
Concerning incominv RADIUS requests leveraging authentication modules, some authentication modules clearly can not be
...
session such as can be done with cookies on Http clients. Any module that depends on use of an HttpServletRequest or
HttpServletResponse object without verifying that they actually have those objects will result in exceptions being
thrown and authentication to rejected.
...
different templates and different flows through the module to remove those features that are not applicable or even adapt
to the different interactions afforded by RADIUS clients.
...
There is also support for launching via a SpringFramework component-scan directive in a servlet file. This web.xml
declaration results in the RADIUS server log entries appearing in catalina.out assuming a tomcat deployment. At startup
you'll see the following lines appear. Some parts have been snipped out in the content below to make it more clear:
...
you can now define RADIUS Clients, set the port on which to listen for requests, and enable the RADIUS server. For now,
I'll just enable the RADIUS Server using the radius button in the page and press the Save button.

Whenever this configuration for the RADIUS server changes you'll immediately see log entries in catalina.out that show
the RADIUS server configuration being reloaded. Now that it is enabled, a thread pool also launches as shown:
...
To exercise the RADIUS server that we just started we can use this module's generated jar file to run the included
__ConsoleClient__ command line tool. It allows a user to authenticate to openAM using the RADIUS protocol by providing
simple prompts on the command line and translating user input to corresponding requests to the server. Opena command
shell that has access to java and either change into Open AM's WEB-INF/lib directory or include the full path to the
module's jar file, openam-auth-radius-1.0.1.jar in my case, in the following command. Then run this command:
...
The shared secret must match exactly the value specified for the RADIUS Client configured in Open AM for this particluar
...
use the source IP address of the RADIUS UDP packet to decide which shared secret to use". In other words,  the IP Configured clients are selected whose configured IP address matches the
IP address of incoming packet connecting to the RADIUS server's port. configured in the console
for the ConsoleClient including case.
